...
In the following example, we are providing a very basic full implementation of an audio visualizer. You can easily copy-paste this into an empty `.html` file, import it into Wallpaper Engine and it should work straight away. We don't recommend placing all scripts, styles and HTML content into one file normally, however, for the sake of this tutorial, it's easiest to see it all in one place.
...
If you continue to have issues with your visualizers, we recommend setting up a debugger connection for Wallpaper Engine as outlined in the [Web Wallpaper Debugging](debug.md) tutorial, this way you can debug the JavaScript code more effectively and check the incoming audio levels you receive.
