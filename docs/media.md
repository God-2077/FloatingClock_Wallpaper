...
This event function will be called when the users starts, stops or pauses media playback. You can use the class constants to check for the appropriate state. For example:

```js
event.state == window.wallpaperMediaIntegration.playback.PLAYING
```
